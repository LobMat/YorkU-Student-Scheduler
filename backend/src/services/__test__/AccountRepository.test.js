...
const { AccountRepository } = require('../AccountService');
...
test('AccountUtils.getAccountFromData() #1', async () => {
    createDummyAccounts();
    ...
    const firstLoadAcc = AccountUtils.getFromData(key, firstLoadData);
    ...
    const secondLoadAcc = AccountUtils.getFromData(key, await AccountRepository.readAccount(key));
    ...
    const thirdLoadAcc = AccountUtils.getFromData(key, await AccountRepository.readAccount(key));
    expect(thirdLoadAcc.password).toBe("abcdefg");   
})

// test that the function 'getKey' takes in a username and finds the whole key that matches the username, containing the email as well.
test('AccountRepository.getKey() #1', async() => {
    createDummyAccounts();

    // test that key is generated properly from username
    const calebKey = await AccountRepository.getKey("calebwj");
    expect(calebKey).toBe("calebwj|2005cwj@gmail.com");
    
    // use the key to get data
    const firstLoadAcc = AccountUtils.getFromData(calebKey, await AccountRepository.readAccount(calebKey));
    expect(firstLoadAcc.password).toBe("caleb1234");

    // call the function with a username that doesnt exist.
    const fakeKey = await AccountRepository.getKey("zaynAli");
    expect(fakeKey).toBe(undefined);
    
})

// test that the function 'getKey' takes in a email and finds the whole key that matches the email, containing the username as well.
test('AccountRepository.getKey #2', async() => {
    createDummyAccounts();

    // test that key is generated properly from username
    const calebKey = await AccountRepository.getKey("2005cwj@gmail.com");
    expect(calebKey).toBe("calebwj|2005cwj@gmail.com");
